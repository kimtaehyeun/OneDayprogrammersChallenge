...
등수가 높은 3명을 선발해야 하지만, 개인 사정으로 전국 대회에 참여하지 못하는 학생들이 있어 참여가
 가능한 학생 중 등수가 높은 3명을 선발하기로 했습니다.
...
public class chellenge1 {
	...
	public static int solution(int[] rank, boolean[] attendance) {
		...
		for(int i=1; i<=rank.length; i++){
			int memberNum =getIndex(rank, i);
			if(attendance[memberNum]) {
				answer +=memberNum*value;
				value = value/100;
				count++;
				if(count==3) {
					return answer;
	...
	public static int getIndex(int[] rank, int getNum) {
